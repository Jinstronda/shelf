...
export function ShareCardModal({ title, authors, coverUrl, rating, review, shareExcerpt, onClose }: Props) {
  ...
  useEffect(() => {
    if (!coverUrl) return
    let cancelled = false
    fetch(`/api/cover-proxy?url=${encodeURIComponent(coverUrl)}`)
      .then(res => res.ok ? res.blob() : null)
      .then(blob => {
        if (!blob || cancelled) return
        const reader = new FileReader()
        reader.onloadend = () => { if (!cancelled) setCoverDataUrl(reader.result as string) }
      ...
      <div onClick={e => e.stopPropagation()} style={{
            ...
            <div key="story" ref={cardRef} style={{
              ...
              <div style={{
                ...
                {(coverDataUrl || coverUrl) && (
                  <img src={coverDataUrl || coverUrl!} alt=""
        ...
        <div style={{ padding: '12px 16px 16px', display: 'flex', gap: 8 }}>
          ...
          <button onClick={handleCopyLink} style={{
            background: linkCopied ? '#2a5a3a' : 'rgba(255,255,255,0.06)',
            color: linkCopied ? '#fff' : '#9ab', border: 'none',
